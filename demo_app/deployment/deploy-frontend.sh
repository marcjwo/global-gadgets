#!/usr/bin/env bash

# Load env variables
source ./env.sh

if [ -z "$REGION" ]; then
  echo "REGION is not set. Please set the gcloud run/region."
  exit 1
fi

# Get the latest tags.
git fetch

#
# Build & push the container
#
echo "Building and pushing the container"
TAG_NAME=$(git describe --abbrev=0 --tags --always)
IMAGE=$REGION-docker.pkg.dev/$PROJECT_ID/global-gadgets-repo/global-gadgets-repo:$TAG_NAME

sudo docker build --rm -t "$IMAGE" .
sudo docker push "$IMAGE"

#
# Step 3: Deploy to Cloud Run
#
echo "Deploying to Cloud Run"
gcloud beta run deploy global-gadgets \
  --image="$IMAGE" \
  --execution-environment=gen2 \
  --cpu-boost \
  --network="$VPC_NETWORK" \
  --subnet="$VPC_SUBNET" \
  --vpc-egress=private-ranges-only \
  --region="$REGION" \
  --project="$PROJECT_ID" \
  --set-env-vars=PGHOST="$PGHOST",PGPORT="$PGPORT",PGDATABASE="$PGDATABASE",PGUSER="$PGUSER",PGPASSWORD="$PGPASSWORD",PROJECT_ID="$PROJECT_ID",REGION="$REGION",GOOGLE_API_KEY="$GOOGLE_API_KEY",BIGQUERY_PROJECT_ID="$BIGQUERY_PROJECT_ID",BIGQUERY_DATASET_ID="$BIGQUERY_DATASET_ID",BIGQUERY_LOCATION="$BIGQUERY_LOCATION"